...
class Products extends Model
{
     ...
     *                  '
     */
    public function ListProducts1($request)
    {
        # code...
        if(!isset($request[['search_text']])){
            return View('products1', ['products' => [], 'measureunits' => []]);
        }
        $SearchText = isset($request['search_text']) ? $request['search_text'] : "";
        try {
            //code...
            ...
            $MeasureUnits = (new MeasureUnits())->where('id', '>', -1)->get();
            return View('products1', ['products' => $Products, 'measureunits' => $MeasureUnits]);
        } catch (\Throwable $th) {
            //throw $th;
            $Message = $this->ErrorInfo($th);
            return Redirect("/error/$Message");
    ...
    public function CreateProduct($request)
    {
        ...
        try {
            ...
            if(count($Products) > 0){
                return ['status' => 'exist', 'element_tag' => $ElementTag, 'isError' => true, 'message' => 'THIS PRODUCT ALREADY EXISTS'];
            }
            //code...
            ...
            return ['status' => 'ok', 'product' => $Product, 'element_tag' => $ElementTag, 'isError' => false, 'message' => 'PRODUCT SUCCESFULLY CREATED'];
        } catch (\Throwable $th) {
            //throw $th;
            $Message = $this->ErrorInfo($th);
            return ['status' => 'error', 'message' => $Message, 'element_tag' => $ElementTag, 'isError' => true, 'message' => $Message];
        }
    }
